	...
	public static void register(CommandDispatcher dispatcher) {
		...
		for (GameType gameType : gameTypes) {
			if (gameType != GameType.NOT_SET) {
				try {
					literalBuilder.then(((LiteralArgumentBuilder) Commands.literal(gameType.getName()).executes((context) -> Util.setGameMode(context, Collections.singleton(context.getSource().asPlayer()), gameType))).then(Commands.argument("target", EntityArgument.players()).executes((cmdContext) -> Util.setGameMode(cmdContext, EntityArgument.getPlayers(cmdContext, "target"), gameType))));
				} catch (Exception e) {
					Util.Log().error("Failed to exec command gm");
				}
			}
		}
