...
public class OxygenTankHUD {
	...
	public static void render(Post event) {
		if (event.getType() == ElementType.TEXT) {
			...
			if (Util.isEquipped(mc.player, ItemList.scuba_mask) && OxygenTank.isTank(chest.getItem()) && mc.player.isInWater()) {
				...
				if(minutes == 0 && seconds <= 1){
					...
					display = TextFormatting.YELLOW + "Oxygen: " + minutes + ":" + (seconds == 0 ? "00" : seconds < 10 ? "0" + seconds : seconds);
